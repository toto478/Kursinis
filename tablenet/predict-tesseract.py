...
class Predict:
    ...
    def predict(self, image: Image) -> List[pd.DataFrame]:
        """Predict table content from an image.
    ...
    def _process_tables(self, segmented_tables: np.array) -> List[np.array]:
        """Process segmented tables to apply convex hull and filter by area.
        
        Args:
            segmented_tables (np.array): Labeled table regions.
        
        Returns:
            List[np.array]: Processed tables as binary masks.
        """
    ...
    def _column_to_dataframe(column: np.array, image: Image) -> pd.DataFrame:
        """Convert a segmented column to a DataFrame using OCR.
        ...
        cropped_image = image_array * column_resized[:,:,np.newaxis] 
        pil_image = Image.fromarray(np.uint8(cropped_image))
        
        text = pytesseract.image_to_string(pil_image, config='--psm 6')
        return pd.DataFrame({"col": [line.strip() for line in text.split('\n') if line.strip()]})

@click.command()
@click.option('--root_folder', default=r'C:\Users\Simas\Desktop\Ataskaitos\kursinis\GOOD OCR_tablenet-master\Processed Images -tesseract')
