...
class ProcessingSettings:
    Language = "English"
    OutputFormat = "xml"  # Changed to XML to get detailed layout information
...
class AbbyyOnlineSdk:
	...
	def process_image(self, file_path, settings):
        ...
        def extract_tables(self, xml_content):
            """ Extract only table text from the given XML content """
            dom = xml.dom.minidom.parseString(xml_content)
            text_elements = dom.getElementsByTagName("text")
            table_texts = []

            for text in text_elements:
                style = text.getAttribute("blockType")
                if style == "Table":
                    table_texts.append(text.childNodes[0].data.strip())
