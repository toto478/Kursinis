...
def compute_accuracies(gt_text, ext_text):
    ...
    direct_symbol_matcher = SequenceMatcher(None, ext_text, gt_text, autojunk=False)
    ...
    lcs_symbol_matcher = SequenceMatcher(None, ext_text, gt_text, autojunk=False)
    lcs_symbol_size = sum(block.size for block in lcs_symbol_matcher.get_matching_blocks())
    print(lcs_symbol_size)
